...
| `animations.feed_blob_fps` | `5` | Frame rate of the empty unread feed's drifting blob field. The drift runs in minutes, so 5 is plenty for it; raise it (up to 60) for a smoother poke — clicking a blob shoves it away and swells it. The motion runs on a wall clock, so the rate changes the smoothness and nothing else, and the loop only ticks while the empty feed is on screen. |
