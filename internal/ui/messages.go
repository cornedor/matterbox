...
type teamsLoadedMsg struct {
	teams []*model.Team
	// resync marks a mid-session refetch — see channelsLoadedMsg.resync, which
	// this mirrors for the tab strip.
	resync bool
}
...
// membershipResyncMsg fires at the end of the debounce window opened by
// scheduleMembershipResync — see membership.go.
type membershipResyncMsg struct{}
