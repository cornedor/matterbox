...
func TestRefPanelLinkIsClickable(t *testing.T) {
	...
	got := out.(Model)
	// Link opens on release when there was no drag (text selection shares the
	// same mousedown path as messages/thread/info).
	out, _ = got.handleMouseRelease(tea.MouseReleaseMsg(tea.Mouse{Button: tea.MouseLeft, X: x, Y: y}))
	got = out.(Model)
