...
// terminal resize). gen drops a result the user has since cycled/closed past.
...
func (m Model) loadPreviewImage(gen int, id uint32, it previewItem) tea.Cmd {
	return func() tea.Msg {
		...
		return previewImageLoadedMsg{
			gen: gen, frames: frames, delays: delays, seqs: seqs,
...
func (m Model) handlePreviewTick(msg previewTickMsg) (tea.Model, tea.Cmd) {
	if !m.preview.active || msg.gen != m.previewGen || len(m.preview.frames) <= 1 ||
...
func (m Model) handlePreviewReencoded(msg previewReencodedMsg) (tea.Model, tea.Cmd) {
	if !m.preview.active || msg.gen != m.previewGen || msg.err != nil ||
		len(msg.seqs) != len(m.preview.frames) {
		return m, nil
	}
	m.preview.seqs = msg.seqs
...
// reencodePreview returns a background command that re-builds every frame's
// transmit sequence at the current placement size (capturing it now so a later
