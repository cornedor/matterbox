...
func newRootCmd() *cobra.Command {
	root := &cobra.Command{
		...
		Long: "matterbox is a TUI Mattermost client.\n\n" +
			...
			"(login, send, read, unread, search, channels, digest, whoami, embed) to\n" +
			"work with messages non-interactively for scripting.",
		SilenceUsage:  true,
		SilenceErrors: true,
	...
	root.AddCommand(newLoginCmd(), newURLHandlerCmd(), newRegisterHandlerCmd(), newSendCmd(), newReadCmd(), newUnreadCmd(), newSearchCmd(), newChannelsCmd(), newDigestCmd(), newWhoamiCmd(), newEmbedCmd())
	return root
}
