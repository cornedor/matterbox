...
// shortcode (no colons). The table is Mattermost's own, so the name we react
// with is the one its clients show; the variation selector Telegram may or may
// not send is tolerated on either side.
func mattermostEmojiName(tgEmoji string) (string, bool) {
	return emoji.Name(tgEmoji)
}
