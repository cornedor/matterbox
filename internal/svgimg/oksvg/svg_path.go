...
func (svgp *SvgPath) DrawTransformed(r *rasterx.Dasher, opacity float64, t rasterx.Matrix2D) {
	...
	if svgp.linerColor != nil {
		...
		// matterbox: LineWidth was passed through unscaled while the path's
		// coordinates went through mAdder's matrix, so a stroke inside a scaled
		// group came out at its unscaled width — 14x too thick on a drawing built
		// from a "shrink everything" transform.
		r.SetStroke(fixed.Int26_6(svgp.LineWidth*strokeScale(svgp.mAdder.M)*64),
...
// strokeScale is the factor a transform applies to a stroke width: the square
// root of the absolute determinant, which is the geometric mean of the two axis
// scales. Correct under rotation, and under a flip — where averaging the matrix
// diagonal would come out at zero and lose the stroke entirely.
//
// matterbox addition.
func strokeScale(m rasterx.Matrix2D) float64 {
	det := m.A*m.D - m.B*m.C
	if det < 0 {
		det = -det
	}
