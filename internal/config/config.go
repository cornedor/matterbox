...
type Config struct {
	...
	// inside a message taller than the pane) and the open thread; clicking a
	// team tab or channel switches to it; clicking a message selects it;
	// dragging within a message selects text that's copied on release; and
	// hovering a tab or channel highlights it. Pointer so an absent key defaults
	// to true; set `mouse: false` to keep the terminal's native click-drag text
	// selection / copy, which capturing the mouse otherwise disables (most
	// terminals fall back to shift-drag). See internal/ui.
